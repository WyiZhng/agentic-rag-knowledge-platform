...
{%- endif %}
    ...
    async def _execute(self, statement: Any) -> Any:
        """Execute a statement against either async PostgreSQL or sync SQLite."""
{%- if cookiecutter.use_postgresql %}
...
{%- endif %}
    ...
    async def workspace_stats(self) -> dict[str, Any]:
        ...
        """
        # Total users
        total_users = (await self._execute(select(func.count(User.id)))).scalar_one()
...
{%- if cookiecutter.use_ai %}
        total_conversations = (
            await self._execute(select(func.count(Conversation.id)))
        ).scalar_one()
        total_messages = (
            await self._execute(select(func.count(Message.id)))
        ).scalar_one()
{%- else %}
...
{%- endif %}
        ...
        # Billing metrics are generated only when their backing tables exist.
        credits_30d = 0
        mrr_cents = 0
{%- if cookiecutter.enable_credits_system %}
        try:
...
{%- if cookiecutter.enable_billing %}
        try:
            ...
            mrr_cents = int((await self._execute(stmt)).scalar_one() or 0)
        except Exception:  # noqa: BLE001
...
{%- endif %}
    ...
    async def list_stripe_events(
        ...
        """
{%- if cookiecutter.enable_billing %}
        total = (await self._execute(select(func.count(StripeEvent.id)))).scalar_one()
        rows = (
            await self._execute(
                select(StripeEvent)
...
class AdminService:
    ...
    async def workspace_stats(self) -> dict[str, Any]:
        ...
        # Session persistence is currently implemented only by the SQL backends.
        active_24h = 0
