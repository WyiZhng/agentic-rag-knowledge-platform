...
└── commands/             # Click CLI commands (auto-discovered by `agentic_rag_platform cmd …`)
...
The generated project ships a Click CLI exposed as `agentic_rag_platform` (after `make install`):

```bash
agentic_rag_platform server run --reload          # dev server
agentic_rag_platform db upgrade                   # apply migrations
agentic_rag_platform db migrate -m "message"      # create new migration
agentic_rag_platform user create-admin            # interactive admin creation
agentic_rag_platform rag-ingest <path> -c docs    # ingest local files
agentic_rag_platform rag-search "query" -c docs   # semantic search
agentic_rag_platform rag-collections              # list collections
agentic_rag_platform celery worker                # start worker
agentic_rag_platform celery beat                  # start scheduler
```

Run `make help` for a categorized list, or `agentic_rag_platform --help` for full CLI docs.
...
POSTGRES_DB=agentic_rag_platform
...
# Semantic search
agentic_rag_platform rag-search "your query" --collection documents
```
