...
**agentic_rag_platform** - FastAPI application generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template).

**Stack:** FastAPI + Pydantic v2, PostgreSQL
, JWT + API Key auth, Redis
, langgraph (openai), RAG (pgvector), Next.js 15 (i18n)
...
# RAG
uv run agentic_rag_platform rag-ingest /path/to/file.pdf --collection docs
uv run agentic_rag_platform rag-search "query" --collection docs

# Sync Sources
uv run agentic_rag_platform cmd rag-sources
uv run agentic_rag_platform cmd rag-source-add
uv run agentic_rag_platform cmd rag-source-sync
```
